...
const EditTransaction = () => {
  ...
  useEffect(() => {
    const fetchTransactionData = async () => {
      const dataFromLocalStorage = await axios.get(`${process.env.REACT_APP_API_URL}/order/${transactionId}`);

      if (dataFromLocalStorage) {
        setTableData(dataFromLocalStorage.data);
    ...
    if (!hasEffectRun.current) {
      ...
      const selectedItems = [
        tableData.roller_size,
        tableData.pipe_size,
        tableData.cat,
        tableData.roller_name,
        tableData.cap,
        tableData.poly,
        tableData.nutHandle,
        tableData.spray,
  ...
  const updateOrder = () => {
    ...
    axios
      .put(`${process.env.REACT_APP_API_URL}/placeOrder/${transactionId}`, {
        orders: orderItems,
      })
      .then((response) => {
        console.log("Orders update successfully:", response.data.orders);
